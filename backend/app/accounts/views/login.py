...
class UserFreeLoginView(APIView):
    def post(self, request):
        ...
        if not user:
            raise ValidationError({"message": "There is no free account available in the current system"})
        request.user = user
...
class AccountLogin(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.expired_date and user.expired_date <= timezone.now().date():
            raise ValidationError({"message": "Account expired"})
...
class AccountRegister(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if not ALLOW_REGISTER:
            raise ValidationError({"message": "The current system prohibits account registration"})
