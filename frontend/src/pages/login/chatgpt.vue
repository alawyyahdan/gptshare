<template>
  <div>
    <t-dialog
      ...
      header="Silakan pilih akun ChatGPT"
      :cancel-btn="null"
      :confirm-btn="null"
      :on-close="onClose"
      width="930px"
          ...
          >
            ...
            <template #action>
              <div>
                ...
                <t-tag v-else-if="item.use_count.last_1h + item.use_count.last_2h + item.use_count.last_3h < 20" theme="success" variant="light"> Idle </t-tag>
...
const onSelect = async (chatgptId: number) => {
  ...
  if (response.status !== 200) {
    ...
    MessagePlugin.success('Login sukses!');
