    ...
    <t-card class="list-card-container">
      ...
      >
        <template #is_active="{ row }">
          <t-switch :value="row.is_active" :custom-value="[true, false]">
            <!-- <template #label="slotProps">{{ slotProps.value == 1 ? 'Enable' : 'Deactive' }} </template> -->
          </t-switch>
        </template>
      ...
      <t-dialog v-model:visible="showDialog" :on-confirm="handleConfirm" title="Add User" width="800px">
        ...
        >
          ...
          <t-form-item label="Isolated Session" name="isolated_session">
            <t-switch v-model="newUser.isolated_session" :custom-value="[true, false]" />
          </t-form-item>

          <t-form-item label="Tanggal Expire" name="expired_date">
            <t-date-picker
                ...
                <t-space direction="vertical" :size="3">
                  <div v-for="(line, idx) in newUser.model_limit" :key="line">
                    <t-space :size="5" align="center">
                      ...
                      <span>Limit</span>
                      <t-input-adornment prepend="Setiap" append="Menit">
      ...
      <!-- 确认删除 dialog -->
      <t-dialog v-model:visible="showDeleteDialog" header="Apakah Anda yakin akan menghapus pengguna ini?" width="500" :on-confirm="handleDelete">
      </t-dialog>
    </t-card>
  </div>
</template>
...
const columns: TableProps['columns'] = [
  ...
  { colKey: 'use_count', title: 'Limit Harian', width: 100 },
  { colKey: 'last_login', title: 'Login Terakhir', width: 160 },
  { colKey: 'expired_date', title: 'Expired', width: 160 },
  { colKey: 'date_joined', title: 'Waktu Daftar', width: 160 },
...
const modelLimitValidator: CustomValidator = (val) => {
  for (const item of val) {
    if (!item.model_name) {
      return { result: false, message: 'Model tidak boleh kosong', type: 'error' };
    }
...
const rules: FormProps['rules'] = {
  ...
  username: [
    ...
    { validator: (val) => val.length >= 4, message: 'Setidaknya 4 karakter' },
  ],
...
const batchUpdateUserModelLimit = async () => {
  ...
  if (response.status !== 200) {
    ...
    MessagePlugin.success('Add Limit Sukses! ');
  }
};
